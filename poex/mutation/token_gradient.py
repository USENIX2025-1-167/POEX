...
class MutationTokenGradient(MutationBase):
    def __init__(
        ...
        self.english_word_ids = model_utils.get_usable_english_word_ids(self.attack_model)
        self.space_token_id = self.attack_model.tokenizer.encode(' ')[0]
        print("usable english words number: ", len(self.english_word_ids))
    ...
    def __call__(
        ...
        if not self.is_universal and len(jailbreak_dataset) > 1:
            ans = []
            for instance in tqdm(jailbreak_dataset, desc='Mutating'):
        ...
        if self.avoid_unreadable_chars:
            # 除了英文单词外的token id都设置为负无穷
            for i in range(V):
                if i not in self.english_word_ids:
                    score_tensor[:, :, i] = float('-inf')
        ...
        with torch.no_grad():
            ...
            for _ in range(self.num_turb_sample):
                ...
                # 避免替换空格
                while (rel_idx < new_prompt_ids[0].size(1) and new_prompt_ids[0][0][rel_idx] == self.space_token_id) or \
                      (rel_idx >= new_prompt_ids[0].size(1) and new_prompt_ids[1][0][rel_idx-new_prompt_ids[0].size(1)] == self.space_token_id):
                    rel_idx = random.randint(0, L1-1)
