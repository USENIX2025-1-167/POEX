...
def get_usable_english_word_ids(model):

    # 从文件中读取英文单词列表
    with open('./poex/utils/20k.txt', 'r') as f:
        english_words = set(f.read().splitlines())
    
    # 获取模型的词汇表
    vocab = model.tokenizer.get_vocab()
    
    # 用于存储可用的英文单词的token ID
    usable_english_word_ids = set()
    ...
    for token, token_id in vocab.items():
        # 如果模型名字小写中包含llama
        if model.model_name.lower().find('llama') != -1:
        # 检查开头是不是Ġ
            if token.startswith('Ġ'):
                token = token[1:]
                # 检查token是否匹配单词模式,并且是英文单词
                if word_pattern.match(token) and token in english_words:
                    usable_english_word_ids.add(token_id)
        elif model.model_name.lower().find('mistral') != -1:
            # 检查token是否匹配单词模式,并且是英文单词
            if token.startswith('▁'):
                token = token[1:]
                if word_pattern.match(token) and token in english_words:
                    usable_english_word_ids.add(token_id)
        elif model.model_name.lower().find('vicuna') != -1:
            # 检查token是否匹配单词模式,并且是英文单词
            if token.startswith('▁'):
                token = token[1:]
                if word_pattern.match(token) and token in english_words:
    ...
    return list(usable_english_word_ids)
...
def get_nonsense_token_ids(model):
    ...
    for i in range(model.vocab_size):
        try:
            ...
            if (contains_uninterpretable_characters(s) or 
                s.strip() == '' or 
                s.rstrip() != s or 
                i in model.tokenizer.all_special_ids or 
                contains_control_characters(s) or 
                not is_english_only(s)):
                ans.append(i)
        except:
            continue
    return ans
