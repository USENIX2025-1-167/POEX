...
from poex.constraint.ConstraintBase import ConstraintBase
from poex.datasets.jailbreak_datasets import JailbreakDataset
from poex.datasets import Instance
from poex.models import WhiteBoxModelBase
...
class PerplexityConstraint(ConstraintBase):
    ...
    def __call__(self, jailbreak_dataset, *args, **kwargs) -> JailbreakDataset:
        ...
        """
        # new_dataset = []
        for instance in tqdm(jailbreak_dataset, desc='Perplexity Constraint'):
            seed = self._format(instance)
            constraint_score = self.judge(seed)
            instance.constraint_score = constraint_score
            # if constraint_score < self.threshold:
    ...
    def _format(self, instance: Instance):
        ...
        for attr in self.attr_name:
            ...
            param_attr = param_attr.replace(temp_pattern, getattr(instance, 'query'))
        return param_attr
